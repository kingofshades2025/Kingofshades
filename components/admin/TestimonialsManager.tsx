...
export function TestimonialsManager({ testimonials }: { testimonials: Testimonial[] }) {
  ...
  return (
    <>
      ...
      <form
        action={(fd) => startTransition(() => { void upsertTestimonial(fd); })}
        ...
        {testimonials.map((t) => (
          <div key={t.id} className="rounded-2xl border border-line bg-surface/70 p-5">
            <div className="flex items-start justify-between gap-4">
              ...
              <Button size="sm" variant="ghost" disabled={isPending} onClick={() => startTransition(() => { void deleteTestimonial(t.id); })}>
                Delete
              </Button>
            </div>
